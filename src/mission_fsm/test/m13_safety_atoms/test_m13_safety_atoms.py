...
def test_tc_atom_001_battery_low_cruise_to_rtb(mission_fsm_sil_harness: SimpleNamespace) -> None:
...
def test_tc_atom_004_geofence_breach_cruise_to_event(mission_fsm_sil_harness: SimpleNamespace) -> None:
    _reach_cruise(mission_fsm_sil_harness)
    mission_fsm_sil_harness.inj.inject("geofence_breach", True)
    assert mission_fsm_sil_harness.wait_mode("EVENT")
    assert mission_fsm_sil_harness.spin_until(
        lambda: "geofence_rtb" in mission_fsm_sil_harness.cap.triggers, timeout_sec=3.0
    )
