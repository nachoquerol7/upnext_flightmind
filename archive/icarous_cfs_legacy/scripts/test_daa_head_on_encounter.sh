#!/usr/bin/env bash
# Test de encuentro de frente (head-on) para DAIDALUS/ICAROUS en SITL.
# Criterio PASS: aparece numConflictTraffic >= 1 en /upnext_daa/.../bands_summary.
#
# Uso:
#   source /opt/ros/jazzy/setup.bash && source ~/upnext_uas_ws/install/setup.bash
#   ./scripts/test_daa_head_on_encounter.sh
#
# Variables opcionales:
#   WAIT_PUB_SEC=90       # espera a publishers /fmu/out/*
#   WAIT_CONFLICT_SEC=120 # espera a que aparezca conflicto
set -eo pipefail
set +m
...
echo "[head_on] Parando demos anteriores..."
bash "${ROOT}/archive/icarous_cfs_legacy/scripts/stop_daa_demo.sh" 2>/dev/null || true
sleep 2

LOG=/tmp/upnext_daa_head_on.log
echo "[head_on] Lanzando escenario head-on (headless) ... log: ${LOG}"
...
echo "[head_on] Esperando publishers PX4 (timeout ${WAIT_PUB_SEC}s)..."
t0=$(date +%s)
pub_ok=0
while true; do
  now=$(date +%s)
...
echo "[head_on] Esperando detección de conflicto DAA (timeout ${WAIT_CONFLICT_SEC}s)..."
t1=$(date +%s)
conflict_ok=0
while true; do
  now=$(date +%s)
...
echo "PASS: encuentro head-on detectado por DAIDALUS (numConflictTraffic>=1)."
echo "Log: ${LOG}"
exit 0
