#!/usr/bin/env bash
# Test de cruce perpendicular para DAIDALUS/ICAROUS en SITL.
...
echo "[crossing] Parando demos anteriores..."
bash "${ROOT}/archive/icarous_cfs_legacy/scripts/stop_daa_demo.sh" 2>/dev/null || true
sleep 2

LOG=/tmp/upnext_daa_crossing.log
echo "[crossing] Lanzando escenario cruce perpendicular (headless) ... log: ${LOG}"
...
echo "[crossing] Esperando publishers PX4 (timeout ${WAIT_PUB_SEC}s)..."
t0=$(date +%s)
pub_ok=0
while true; do
  now=$(date +%s)
...
echo "[crossing] Esperando detección de conflicto DAA (timeout ${WAIT_CONFLICT_SEC}s)..."
t1=$(date +%s)
conflict_ok=0
while true; do
  now=$(date +%s)
...
echo "PASS: encuentro de cruce perpendicular detectado por DAIDALUS."
echo "Log: ${LOG}"
exit 0
